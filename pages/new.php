...
if (!empty($Nom) && !empty($Email) && !empty($Date_de_naissance) && !empty($Mot_de_passe) && !empty($Ville) && !empty($Genre)) 
    {   
    $sql_insert = "INSERT INTO membre (nom, date_naissance, genre, email, ville, mdp, image_profil)
                    VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s')";
    $sql_insert = sprintf($sql_insert, $Nom, $Date_naissance, $Genre, $Email, $Ville, $Mot_de_passe, $image_profil);
